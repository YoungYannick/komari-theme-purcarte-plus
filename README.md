...
- **桌面端背景图片链接** (`backgroundImage`)
  ...
  - **说明:** 支持多张背景图片或图片api，使用“,”分割，多项会按伪随机顺序轮换；使用“|”分隔亮色模式和暗色模式，填写单个则同时用于亮暗模式
...
- **桌面端视频背景链接** (`videoBackgroundUrl`)
  ...
  - **说明:** 视频背景链接，多个使用“,”分割并按伪随机顺序轮换；使用“|”分隔亮色模式和暗色模式，填写单个则同时用于亮暗模式，建议使用无声视频，且视频文件较大时可能会影响加载速度

- **移动端视频背景链接** (`videoBackgroundUrlMobile`)
  - **类型:** `string`
  - **默认值:** `(空)`
  - **说明:** 移动端视频背景链接，多个使用“,”分割并按伪随机顺序轮换，与桌面端一样区分亮暗模式，留空则使用桌面端视频
...
- **免费标签文本** (`freeTag`)
  - **类型:** `string`
  - **默认值:** `白嫖中`
  - **说明:** 用于识别免费/白嫖节点的标签文本。资产统计的排除免费标签、高级搜索的搜索免费都会按此文本精确匹配节点 `tags`（分号分隔）
...
- **启用延迟总览** (`enablePingOverview`)
  - **类型:** `switch`
  - **默认值:** `true`
  - **说明:** 启用后在标题栏显示延迟总览入口按钮，关闭后 /ping-overview 页面也将无法访问
...
- **延迟图表最大渲染点数** (`pingChartMaxPoints`)
  - **类型:** `number`
  - **默认值:** `0`
  - **说明:** 设置延迟图表的最大渲染点数，0 表示使用自动智能降采样（根据数据量和线条数自动计算最佳点数，使用 LTTB 算法保留视觉形状），设置正整数则强制使用该值。此配置只控制前端渲染层，历史源数据仍受 Komari 后端存储精度影响；官方最新后端默认按约 15 分钟原始数据、48 小时内 1 分钟、14 天内 5 分钟、更早 1 小时的层级提供数据。动态快捷范围会避免让中间范围刚好越过 14 天精度边界，例如保留 30 天时显示 14 天和 30 天
