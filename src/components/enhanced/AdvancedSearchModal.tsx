      ...
      >
        ...
        <div className="advanced-search-content">
          ...
          <div className="search-section">
            ...
            {/* 交换空间：允许为0，单位 MB/GB */}
            <RangeSearchField<SwapUnit>
              fieldKey="swap_total"
              state={state.swap_total}
              onChange={(val) =>
                setState((prev) => ({ ...prev, swap_total: val }))
              }
              units={["MB", "GB"] as SwapUnit[]}
              errors={validationErrors}
              t={t}
            />
...
/**
 * 范围搜索字段（mem_total/disk_total/swap_total/traffic_limit）
 * 作用：两个数字输入框（from/to）+ 单位下拉
 *   - 只填 from：大于等于
 *   - 只填 to：小于等于
 *   - 两个都填：范围内
 * 限制：
 *   - mem_total/disk_total 不允许为 0
 *   - swap_total/traffic_limit 允许为 0
 *   - 小数最多精确到两位
 *   - to 必须大于 from
 *   - 输入框最大长度 16，防止 JS 溢出
 *   - 为空则不参与搜索
